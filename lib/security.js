...
const VALID_STEPS_ADMIN = new Set([
  ...
  'cancel-subscription', 'request-otp', 'verify-otp',
]);

export function validateAdminInput(body) {
  const { step, ownerId } = body || {};
  if (!step || typeof step !== 'string') return 'Campo step obrigatório';
  if (!VALID_STEPS_ADMIN.has(step)) return `step inválido: ${step}`;
